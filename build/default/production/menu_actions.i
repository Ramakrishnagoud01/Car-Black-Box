...
void view_log(unsigned char key)
{
    ...
    if(key == 0x3D && read_addr >= 10)
        read_addr -= 10;
    else if(key == 0x3E && read_addr < (event_count - 1) * 10)
        read_addr += 10;
...
void clear_log(void)
{
    ...
    if(once)
    {
        ...
        clcd_print("LOGS ARE CLEARED", (0x80 + 0));
        clcd_print("                ", (0xC0 + 0));
...
void down_log(void)
{
    ...
    if (once)
    {
        clcd_print("  THE LOGS ARE  ", (0x80 + 0));
        ...
        clcd_print(" DOWNLOAD DONE ", (0xC0 + 0));
        once = 0;
        delay = 0;
    }
...
void change_pass(unsigned char key)
{
    ...
    if(flag == 0)
    {
        ...
        if(i < 4)
        {
            ...
            {
                prev_sec = sec;
                clcd_putch((sec % 2) ? ' ' : 0xFF, (0xC0 + i + 3));
            }

            if ((key == 0x3E || key == 0x3D) && key != prev_key)
            {
                prev_key = key;
                clcd_putch('*', (0xC0 + i + 3));
                arr1[i++] = (key == 0x3E) ? '0' : '1';
        ...
        if(i == 4 && j < 4)
        {
            ...
            {
                prev_sec2 = sec;
                clcd_putch((sec % 2) ? ' ' : 0xFF, (0xC0 + j + 3));
            }
    ...
    if(flag == 1)
    {
        ...
        {
            my_strcpy(pass, arr1);
            clcd_print("PASSWD IS CHANGED", (0x80 + 0));
        }
        else
        {
...
void set_time(unsigned char key)
{
    ...
    if (rtc_sec != prev_sec)
    {
        ...
        unsigned char pos = (field == SET_HR) ? 3 :
                            (field == SET_MIN) ? 6 : 9;
        clcd_putch((rtc_sec % 2) ? ' ' : 0xFF, (0xC0 + pos));
    }
